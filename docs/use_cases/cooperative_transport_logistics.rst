...
During the development of the SMIA Operator extension, a dashboard was designed to facilitate the discovery of available SMIAs and the user's use of transport requests to those SMIAs. The dashboard is divided into three sections (External SMIAs loader, Available capabilities and skills, and SMIA candidates), following the workflow described in the :ref:`SMIA ecosystem SMIA Operator` guide.

The following figure shows the dashboard as displayed in this use case, with two transport robots available and the ``EfficientTransport`` capability selected:
...
When the capability request is completed, the execution information is shown on a results page with a timeline of the performed steps:
...
.. seealso::
    ...
    For a detailed explanation of the dashboard layout, the complete execution workflow, and installation instructions, refer to the :octicon:`repo;1em` :ref:`SMIA ecosystem SMIA Operator` guide.
