...
The first step is the establishment of an appropriate deployment environment. There are two primary types of environments for the SMIA Operator.
...
#. Specify the path for the ``SMIA_Operator.aasx`` model within the starter script. Verify that the model is located in ``smia_archive/config/aas``.
...
.. tip::
    ...
    The Docker image is generated through a Dockerfile that incorporates the necessary files and defines the entry point of the agent (see :numref:`fig:smia-use-case:operator-dockerfile`).
...
- Verify that the ``SMIA_Operator.aasx`` model is located in the ``aas/`` directory.
...
Following the successful instantiation of the agent containers, their performance and behavior can be validated. The SMIA Operator autonomously orchestrates the underlying FIPA-SMIACL interactions.
