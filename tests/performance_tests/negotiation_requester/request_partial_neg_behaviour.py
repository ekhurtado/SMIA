...
class RequestPartNegBehaviour(CyclicBehaviour):
    ...
    async def on_start(self):
        ...
        self.cfp_msg_body = await acl_smia_messages_utils.generate_json_from_schema(
        ...
        _logger.info("Creating the execution queue considering iterations, agents involved and parallel negotiations...")
        self.execution_queue = deque()  # We use deque for efficient extraction (O(1)) from the left
            ...
            for neg_iter in range(1, self.parallel_negotiations + 1):
                ...
                # for smia_instance_id in self.smia_instances_ids:
                    ...
                    acl_msg_info = {
                        ...
                        'thread': cfp_thread,
                        'experimentIter': experiment_iter,
                        'negIter': neg_iter,
                        'is_last': (index == len(self.smia_instances_ids) - 1)
                    }
                    self.execution_queue.append(acl_msg_info)
            self.execution_queue.append({
                'type': 'WAIT_CYCLE',
                'duration': self.req_cycle_time,
    ...
    async def run(self):
        ...
        if current_msg_info['type'] == 'WAIT_CYCLE':
            ...
            _logger.aclinfo("FIPA-CNP to thread [{}] initiated with SMIA candidates {}"
                            .format(current_msg_info['thread'], self.smia_instances_ids))
            self.myagent.requested_negs_threads.add(current_msg_info['thread'])
            self.myagent.requested_negs_dict[current_msg_info['thread']] = {
